...
# Issues
# Author does not uses this layer on train.py -> config? exclude this file?
...
class Convt2d(nn.Module):
    ...
    def __init__(self, ch_out, ch_in, kernel_h_sz, kernel_w_sz, stride, padding):
        ...
        self.b = nn.Parameter(torch.zeros(ch_in))
        nn.init.xavier_uniform_(self.w)


    def forward(self, x):
        x = F.conv_transpose2d(x, self.w, self.b, stride = self.stride, padding = self.padding)
        return x
